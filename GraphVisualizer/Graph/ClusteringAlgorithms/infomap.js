...
let jInfomap = function (nds, edgs, __MIN) { // A function expression can be stored in a variable. After it has been
    ...
    // ----------------------------------------- Algorithm -----------------------------------------
    function init_status(graph, status) { // Aim of this function is to initialize network properties after Louvain
    ...
    function __mdl(status) { // It is possible to calculate network's modularity only using graph.status.
    ...
    function __one_level(graph, status) { // Computes one level of the communities dendogram (without including community aggregation).
        ...
        while (modif) { // This cycle is not the one that removes or inserts nodes.
            ...
            if (cur_mdl - new_mdl < __MIN) { // Even if best_com !== com_node, if new_mdl - cur_mdl < __MIN
                // cycle is broken.
                break;
            }
        }
